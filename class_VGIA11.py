    ...
    def __init__(self, initial_balance = 1000.0000) -> None:
        ...
        try:
            with open(VGIA11.json_file, 'r') as file:
                ...
                # Filtrar por fecha
                df_filtered = df_time_series[df_time_series.index >= VGIA11.filter_date]
        ...
        for i in range(len(df_sorted)):
            ...
            if dividend_amount > 0:
                ...
                if only_first_time == True:
                    ...
                    cant_vgia11 = int_part
                    balance = decimal_part * close_value
                    only_first_time = False
                    print(f"Inicio(Compra Inicial VGIA11) - Balance: {balance} VGIA11={cant_vgia11}")
                cant +=1
                
                dividend = dividend_amount * cant_vgia11
                balance = balance + dividend
                dividend_vgia11 += dividend
                ...
                cant_vgia11 += int_part
                balance = decimal_part * close_value
                print(f"Recompra de {int_part} cuotas - Balance: {balance} VGIA11={cant_vgia11} Proventos = {dividend}")
                

        print("----------")
        print(f"FII: VGIA11 Cantidad de Meses({cant})")
        saldo_final = balance + cant_vgia11 * close_value 
        total_dividend = dividend_vgia11 
