    ...
    def __init__(self, initial_balance = 1000.0000) -> None:
        ...
        try:
            with open(RURA11.json_file, 'r') as file:
                ...
                # Filtrar por fecha
                df_filtered = df_time_series[df_time_series.index >= RURA11.filter_date]
        ...
        for i in range(len(df_sorted)):
            ...
            if dividend_amount > 0:
                ...
                if only_first_time == True:
                    ...
                    cant_rura11 = int_part
                    balance = decimal_part * close_value
                    only_first_time = False
                    print(f"Inicio(Compra Inicial RURA11) - Balance: {balance} RURA11={cant_rura11}")
                cant +=1
                
                dividend = dividend_amount * cant_rura11
                balance = balance + dividend
                dividend_rura11 += dividend
                ...
                cant_rura11 += int_part
                balance = decimal_part * close_value
                print(f"Recompra de {int_part} cuotas - Balance: {balance} RURA11={cant_rura11} Proventos = {dividend}")
                

        print("----------")
        print(f"FII: RURA11 Cantidad de Meses({cant})")
        saldo_final = balance + cant_rura11 * close_value 
        total_dividend = dividend_rura11 
