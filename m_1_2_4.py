...
def calculate_total(json_file, filter_date):
    ...
    for i in range(len(df_sorted)):
        ...
        if dividend_amount > 0:
            ...
            # Compra LIFE11
            target_date = df_sorted.index[i]
            data = get_data("LIFE11.json")
            price = get_price(data, target_date)
            div = balance/price
            decimal_part, int_part = math.modf(div)
            cant_life11 = int_part
            balance = decimal_part * price
            
            print(f"Compra de {cant_life11} LIFE11 a {price} -> Balance = {balance}")
            ...
            balance = balance + cant_life11 * value + dividend * cant_life11
            dividend_life11 += (dividend * cant_life11)
            
            print(f"Venta LIFE11 - Balance = {balance} -> ({cant_life11} * {value} + {dividend * cant_life11}) Cantidad * Precio + Provento")
            cant_life11 = 0
    ...
    saldo_final = balance + cant_ncra11 * close_value + cant_agrx11 * price + cant_life11 * price
    total_dividend = dividend_ncra11 + dividend_agrx11 + dividend_life11
    ...
    print("----------")
    print("Dividend LIFE11:", dividend_life11)
